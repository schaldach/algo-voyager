    ...
    return (
        <div className="sorting">
            ...
            <div className="algobuttons">
                <button className={animationRunning || animationBlock ? 'disabledbutton' : ''} onClick={runAlgorithm}>Sort</button>
                <button className={animationRunning || animationBlock ? 'disabledbutton' : ''} onClick={() => { if (!animationRunning && !animationBlock) { changeArray(shuffleArray(currentArray)) } }}>Shuffle</button>
                <button className={!animationRunning || animationBlock ? 'disabledbutton' : ''} onClick={() => { if (animationRunning && !animationBlock) { stopAnimation(true); scaleArray(currentArray.length); setTimeout(() => { startAnimation(false); stopAnimation(false) }, 500) } }}>Reset</button>
